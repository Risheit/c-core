...
/**
 * Metadata needed for an arena.
 */
#define ARENA_META_SIZE ((size_t)64)
...
typedef enum std_arena_flags {
  ...
  ARENA_CONT_ON_ALLOC_FAIL = 1 << 1, // Stops this arena from panicking if
                                     // there is not enough memory to allocate
                                     // a pointer. A NULL pointer is returned
                                     // instead.
} std_arena_flags;
 ...
 * Remarks:
 *
 * * Dynamically allocated arenas will allocate ARENA_META_SIZE more than is
 * specified in [size] internally, for arena metadata.
 *
 ...
 * backing memory is not aligned to arena alignment, the arena may not be able
 * to access the entire memory. For example: assume an alignment of 8 bytes, and
 * a backing memory of 7 bytes. If we attempt to allocate 3 bytes then 4, the
 * second allocation will fail, as the arena will add 5 bytes of padding to the
 * 3 byte allocation. Contrast this with a single 7-byte allocation, which will
 * succeed.
 *
 ...
 * invalid, the function's behaviour is undefined.
 *
 ...
 *
 * Arenas with externally-managed buffers cannot be resized.
 */
